...
コンパイルすると`.mlmodelc`が作成されます。場所は`FileManager.default.temporaryDirectory`が返すディレクトリです。作成先のディレクトリはiOS 15とiOS 17で違いがなかったので、今後iOSの新しいバージョンが登場しても変化しないものと思われます。

### 10. `.mlmodelc`ファイルのサイズは？

記事冒頭で紹介したサンプルアプリの場合、192 byteでした。iOS 15とiOS 17で違いはありませんでした。

### 11. `.mlmodelc`を使いまわして問題ない？

問題ありません。一度コンパイルすれば、`mlmodelc`を利用することでコンパイルの時間をスキップできます。
