...
それでは、3つのWorkerを作成しましょう。名前は適当につけました。各自でお好みの名前に変更いただいて構いません。

1. りんごWorker（`name = "apple"`）
2. バナナWorker（`name = "banana"`）
3. さくらんぼWorker（`name = "cherry"`）

appleとbananaは非公開のWorkerです。インターネットからアクセスできません。

cherryはインターネットに公開されるWorkerです。リクエストを受け取ると、appleとbananaへリクエストを中継します。その後、appleとbananaから受け取ったレスポンスを合成して、cherryのレスポンスとして返します。

Cloudflare公式の用語ではありませんが、非公開のWorkerにリクエストを中継するWorkerをGateway Workerとよびます。今回はcherryがGateway Workerに該当します。

### Workerの作成手順

それぞれのWorkerを作成するには、まず`npm create cloudflare@latest`を実行してください。テンプレートは「Hello World" Worker」を選びます。

その後、wrangler.tomlとsrc/worker.tsを編集してください。説明の都合で実装言語としてTypeScriptを選びましたが、もちろんJavaScriptでもService bindingsは利用できます。

以下、3つのWorkerについて作成手順を説明します。
...
ここからは、さくらんぼWorkerのwrangler.tomlの設定とsrc/worker.tsの実装について解説します。
...
ただし、有料プランの場合、BundledとUnboundでコストに差が生じます。以下、公式ドキュメントから引用します。
...
UnboundはWorkerの利用時間をもとに課金されるプランです。Worker間のリクエスト数に関係なく、利用時間を元に課金されます。例えばWorker Aが1 ms、Worker Bが2 ms利用時間を消費するとして、AがBを呼び出すと利用時間は3 ms消費したものと見なされます。Aが3 ms、Bが2 ms、合計5 msとして重複課金されることはありません。
