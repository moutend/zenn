...
従って、JavaScriptの非同期処理をうまく利用すれば、10 msの制限の中でもある程度複雑な処理が行えます。例えばWorkerからHTTPリクエストを送信する、プロキシのようなことも実現できます。
...
とはいえ、仮に上記のUint8Arrayに画像データのピクセルが格納されているとして、核ピクセルごとに何らかの計算を行えばCPU利用時間の制限に引っかかるはずです。やはりメモリを多く消費する処理の実行基盤としてCloudflare Workersは不向きです。

この点はCloudflareも承知しているようです。例えば画像のリサイズと最適化を行うフルマネージドなサービスとしてCloudflare Imagesが提供されています。

- [Cloudflare Images | Store and Deliver Images at Scale | Cloudflare](https://www.cloudflare.com/products/cloudflare-images/)

少ないメモリをやりくりするのは腕の見せ所ですし、技術的に面白いところではありますが、要件にフィットするなら素直にマネージドサービスを利用する方がお手軽かつ安上がりです。
