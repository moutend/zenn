...
### 2. 訓練中のエラー「Must allow 2048-element vector as output」
...
画像の解像度に制限はありません。ただし、訓練データの画像は299 x 299の正方形に拡大または縮小された後に学習されます。MLModelのインスタンスを`print()`で表示すると学習に使用された入力データの形式が確認できます。
...
Core ImageフレームワークのCIContextで書き出せるフォーマットであれば対応していると

### 3. 訓練用の画像を作成中に「fopen failed for data file: errno = 2 (No such file or directory)」が表示される
...
エラーが発生しているように見えますが、エラーではありません。紛らわしいのですが、このエラーは無視して問題ありません。
