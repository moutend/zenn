...
2. **携帯電話の基地局の違い**: iPhone 13はdocomoの基地局、iPhone SE 第2世代はSIMなしで基地局を検索中のステータスでした。接続先の基地局によって一推定や補正の動作が変化するのであれば、結果に差が生じることになります。
...
土木建築やけのAndroidタブレットにRTKモジュールを搭載した製品があります。
...
ただし、iPhoneで高精度な位置情報が得られるようになるとプライバシーの問題が懸念されます。すでにAppleは正確な位置情報が得られるアクセサリーとしてAirTagを販売していますが、UWBを用いてセンチメートル単位で場所を特定するにはiPhoneからAirTagへ近づく必要があります。言わば受動的な方式です。一方、iPhone単体で高精度な位置情報が得られるようになればストーカー行為のリスクなど危険性が跳ね上がります。

やはり、付加価値とコストやリスクが釣り合わないので、センチメートル単位で高精度な位置情報が取得できる未来が訪れる可能性は低いかもしれません。
