...
1. 本記事で説明するのはApp Storeの公開手順のみです。アプリの開発方法については説明しません。
2. iOSネイティブアプリを前提に説明します。Flutterなどのクロスプラットフォーム対応フレームワークを用いて開発されている場合、参考にならないかもしれません。
3. Apple Developerの登録手順については説明しません。登録が完了した後に本記事を読み進めてください。
4. 無料アプリについて説明します。有料アプリの公開手順については説明しません。
5. 記事の公開後にApple Developer / App Store Connectサイトの外観が変更されるかもしれません。そのため、あえてスクリーンショットは掲載していません。
...
はじめてアプリを開発される場合、最初からXcode Managed Profileが利用可能な状態になっているはずです。確認方法は次のとおりです。
...
証明書やプロファイルの管理でトラブルが発生している場合、申し訳ありませんが解説の範囲外です。各自で対処した後、App Store公開手順に進んでください。
...
アプリの申請には画面サイズ5.5インチと6.7インチのスクリーンショットが必須です。スクリーンショットの解像度が一致していれば、シミュレーターはどの機種を選んでも構いません。枚数はそれぞれ1枚以上必要です。
...
シミュレーターが英語で表示されてしまう場合は言語を変更してください。手順は以下のとおりです。
