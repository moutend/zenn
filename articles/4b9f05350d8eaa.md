...
現時点ではRealtimeKitのドキュメントはdyte-io/docsリポジトリで管理されています。正式発表と同時にcloudflare/cloudflare-docsに移動されるかもしれません。
...
#### Cloudflare Realtimeはビデオ通話サービスではない

WebRTCの説明に進む前に警告します。ここまでの説明を読んで、Cloudflare Realtimeはビデオ通話サービスであると理解されたかもしれませんが、それは間違いです。あくまでビデオ通話は構築可能なサービスの一例であり、ビデオ通話専用のサービスではありません。  
Cloudflare Realtimeが提供するのはリアルタイム通信機能を構築するための基礎となる部品のみです。例えば、以下の機能は提供されません。  
