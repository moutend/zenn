...
具体的な実装へ進む前に、App Store Connectにおけるサブスクリプション関連用語を整理します。理解が曖昧なま実装を進めると後々手戻りが発生しか寝ません。  
...
なお、App Store Connectのプラン一覧画面で並び替えを行うと、自動的にレベルの数値が割り振られます。手作業で設定する必要はありません。また、サブスクリプションを契約中のユーザーが存在していても、任意のタイミングでレベルを変更できます。  
...
## 第3章 有料アプリを公開するための事前準備

そろそろ実装へ進みたいと思われるかもしれませんが、次は有料アプリを公開するための事前準備を行います。これから行う作業は必ずしもアプリの開発前に行う必要はありませんが、事前に済ませておくと時短になります。  
