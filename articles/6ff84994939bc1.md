...
ここから本題です。io.Writerの末尾に文字列Helloを書き込むAppendHello関数を実装したとします。
...
テーブル駆動テストを並列に実行すると失敗する原因が判明しました。`t.Run()`に渡した`f`がforループ内でgoroutineとして実行されるるため、意図しない結果になるのです。
...
`go vet`を実行すれば実装のミスに気がつくことができます。例えば、修正前の並列版TestAppendHelloの実装には以下のような警告が表示されます。
