...
記事を読み進める上で機械学習の知識は不要です。訓練済みモデルを利用するだけならば、簡単に機械学習をアプリケーションに組み込めるのだと実感してもらうのが記事の目的になります。
...
どうです、それほど難しくないでしょう？訓練済みモデルというのは予測ができる準備が整ったモデルという意味です。
...
以下のコードを実行することで識別しの一覧を表示できます。`list.swift`など適当な名前で保存してください。
...
以下のように実行します。大量の文字が表示されるので`jq`で整形して`sort`します。
...
私の環境では303種類の識別しが表示されました。楽器から生活音まで幅広く識別できることがわかります。
