...
本当に世界中のサーバーにデプロイされるのか確認しましょう。cfプロパティにはリクエストが到着したデータセンターを識別する`colo`が用意されています。
...
引用のとおり、`colo`に格納される値はデータセンター最寄りの空港を示す文字列です。
...
| us-east-1a | 44 ms | 32 ms | 226 ms※ | 38 ms | 34 ms |

仮にCloudflare Workersのデプロイ先が米国であれば、日本（ap-northeast-1a）からリクエストを送信してレスポンスを受信するまでに太平洋を往復することになります。しかし、リクエストの送信元に関わらずレイテンシーは40 ms前後でほぼ均一です。

この結果から、リクエストの送信元に最も近いデータセンターが選択されており、それらのデータセンターで同じコードが動作していることが確認できます。

※us-east-1からCloudflare Workersにリクエストを送信すると、まれに100 msから200 ms程度のレイテンシーが発生しました。どのような条件下でレイテンシーが増えるのかは特定できませんでしたが、30回ほどリクエスト送信を試して1回発生するかしないか程度でした。また、30回以上連続してリクエストを送信しても40 ms前後でレイテンシーが安定している場合もありました。リクエストがbotによるDDoSかどうか判定しているのかもしれません。
...
V8の実装を確認したわけではないので推測になりますが、各ループのスコープから抜けるたびに変数`a`のリファレンスカウントは0になるはずです。そうすると誰からも参照されていないのですから、その時点でメモリは未使用と見なされます。GCによって回収されるまで本当の意味でメモリが解放されたとは言えないかもしれませんが、使用中と見なされるメモリの容量が128 MBを下回っていれば処理は継続できるようです。

とはいえ、仮に100 MBのUint8Arrayに画像データのピクセルが格納されているとして、核ピクセルごとに何らかの計算を行うとします。無料プランの制限を回避するにはピクセルあたり10 ns未満にCPU利用時間を抑える必要があります。さらにその処理を3回ループするなら1回あたり約3 ns未満に抑える必要があります。

## AWS LambdaやGCP Cloud Functionsとの比較

Cloudflare Workersと似たコンセプトのサービスとしてAWS LambdaやGCP Cloud Functionsがあります。これらのサービスとの違いを比較します。

### コンテナ仮想化ではない

AWSやGCPはコンテナ仮想化を利用しています。一方、JavaScriptランタイムのV8を実質的な仮想化ソフトウェアとして利用しているのがCloudflare Workersの特徴です。

従って、コンテナの起動を待つ必要がないため、Cloudflare Workersにはコールドスタートとホットスタートの概念がありません。言うならば常にホットな状態です。
...
AWSやGCPのサービスは目的に応じた性能を得るのが用意です。性能不足に陥ったら課金することで解決できます。

一方、Cloudflare WorkersはCPU利用時間の制限とメモリ容量不足の二重苦を抱えています。さらに、記事執筆時点ではメモリ容量128 MBの制限は無料プランと有料プランで同じです。いくら課金しても性能不足を解決できません。

Cloudflare Workersは軽いタスクを素早く終わらせるのが得意です。そのため、CPUとメモリを多く消費するタスク、例えば画像処理の実行基盤としてCloudflare Workersは不向きです。あくまで譬えなので、画像処理をするならGPUを使えというツッコミなしでお願いします。
...
少ない資源をやりくりするのは腕の見せ所ですし、技術的におもしろいところではあります。しかし、要件にフィットするなら素直にマネージドサービスを利用する方がお手軽かつ安上がりかもしれません。

### 言語のサポート

AWSやGCPはコンテナ仮想化を採用しています。そのため、例えばC言語やRustで処理を実装し、カスタムイメージのコンテナをビルドすれば計算機の性能を最大限に引き出すことができます。

一方、Cloudflare WorkersはランタイムにV8を採用しています。JavaScriptか、あるいはTypeScriptのようにJavaScriptへトランスパイル可能な言語の利用が強制されます。

Web Assemblyを利用できれば言語の選択肢は増えるかもしれません。例えばRustはWeb Assemblyをターゲットにしたコンパイルが可能なため、開発言語として採用できる可能性があります。

一方で、Go言語もWeb Assemblyをターゲットにしたビルドが可能ではありますが、GoランタイムとV8ランタイムが共存する形になります。
...
CloudflareのR2やD1が話題になっていることは知っていたのですが、Workersが何者かは知りませんでした。R2やD1を操作するための踏み台的なサービスなのかと想像していたのですが、調べてみるとWorkers自体もかなり強力なサービスであることを知りました。今後、個人でなにかサービスを開発するときはプラットフォームとしてCloudflare Workersが第一候補になりそうです。

この記事がお役に立てたなら幸いです。もし内容に誤りがある場合は知らせていただけると助かります。
