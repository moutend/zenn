...
RealtimeKitのドキュメントが閲覧可能になっているようです。まだCloudflareの公式アナウンスはされていないようですが、リリースが近づいているものと思われます。楽しみですね。  

- [Guides - RealtimeKit Docs](https://docs.realtime.cloudflare.com/guides)

現時点ではRealtimeKitのドキュメントはdyte-io/docsリポジトリで管理されているようです。正式発表と同時にcloudflare/cloudflare-docsに移動されるかもしれませんが。  

### 公式のビデオ通話デモアプリ「Orange Meets」

Cloudflare Realtimeを用いて構築されたサービスを試す方法として、公式のデモアプリ「Orange Meets」は最もお手軽です。以下のURLからアクセスできます。  
...
SDKを使えば、内部でどのようなサービスが組み合わされて機能が実現されているのか気にする必要はありません。しかし、機能不足に直面したとき、SDKの都合で実装されていないのか、Cloudflare Realtimeの仕様で実現できないのか、調査する必要に迫られます。最初にCloudflare Realtimeの概要を把握して損はありません。  
...
その点はCloudflareも承知しているため、より手軽にCloudflare Realtimeを活用してもらうための公式SDKとしてRealtimeKitを開発しているわけです。冒頭で紹介したリリースブログによると、RealtimeKitはiOSやAndroidもサポートするクロスプラットフォームなSDKとして開発されているそうです。  
従って、一般的なビデオ通話機能を安く手軽に実現する目的ならば、Cloudflare Realtimeは適していません。あるいはRealtimeKitが正式リリースされるのを待つべきです。  
また、記事を投稿した時点でRealtimeKitの正式リリースがいつになるのか告知はありません。  

### Cloudflare RealtimeはSTUN / TURN / SFUを提供するフルマネージドなサービスである

ところで、「Cloudflare Realtimeはリアルタイム通信機能を提供するサービスである」そのように説明されても抽象的すぎます。具体的には、何を提供するのでしょうか。  
