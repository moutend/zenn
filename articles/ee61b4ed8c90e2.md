...
今回の測定結果を比べると、iPhone 13で測定した位置情報の精度の低さが目立ちます。これはおそらく経年劣化が原因です。

というのも、iPhone SE 第2世代を購入して半年ほどでiPhone 13を購入し、その後は3年ほど使い続けていたためです。iPhone 13は画面があちこち割れていて、フレームも若干歪むほど酷使しています。iPhone 15 Proは2023年の9月に購入したのですが、技術検証のためときどき使う程度で、ほぼ新品の状態です。

なお、搭載されているGPSの種類はiPhone SE 第2世代とiPhone 13で共通です。iOSのバージョンは位置情報の精度に影響しないはずですから、経年劣化が精度低下の原因ではないかと考えた次第です。
...
ただし、RTKモジュールは非常に高価なため、今後iPhoneに搭載される可能性は極めて低いです。仮に搭載されたとしても、モジュール単独では利用できません。電子基準点からの補正情報をインターネット経由で配信するシステムの運用が必要になるためです。

とはいえ、可能性がゼロとも言えません。例えばCore LocationフレームワークのCLGeocoderは住所文字列を緯度と経度に変換します。この変換処理はオンデバイスではなくクラウドで処理されます。

Appleがその気になれば電子基準点サービスの運用をはじめるかもしれないのです。あるいは電子基準点サービスを提供している企業を買収して、自社のシステムに取り込む手段もあります。ただやはり、得られる付加価値とコストが釣り合わないので、そのような未来が訪れることはないでしょう。
