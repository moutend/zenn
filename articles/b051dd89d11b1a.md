...
記事を投稿した時点では以下のようなpackage.jsonが生成されます。Cloudflareのドキュメントはpackage.jsonのscriptsを修正するように指示されていますが、修正は不要です。
...
Workersで動的にコンテンツがレンダリングされることを確認するため、おみくじコンポーネントを実装してみます。
...
その後、ブラウザで`https://my-next-app.<your_name>.workers.dev/omikuji`を開きます。リロードするたびに大吉・中吉・小吉がランダムに表示されるはずです。ちなみに私の場合、一発目の表示は中吉でした。

## バインディングを利用する

KVやD1などのバインディングは`getCloudflareContext()`で参照できます。試してみましょう。
...
なお、実装にはDrizzle ORMを利用します。素のD1バインディングで読み書きしても構わないのですが、結果をTypeScriptのオブジェクトにマッピングするのが手間なのでORMを利用します。
...
ブラウザで`https://my-next-app.<your_name>workers.dev/d1`を開くとおみくじの履歴が表示されます。何度かリロードして、直近の10件のレコードが取得されていることを確かめてみてください。

## おわりに

軽く試す程度であれば特に不具合には遭遇せず実装できました。が、しかし、現状は実験的機能の扱いなので本番環境へ投入するには今後しばらく様子見したほうがよさそうです。
