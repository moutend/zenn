...
具体的な実装の話題へ進む前に、App Store Connect / StoreKitにおけるサブスクリプション関連用語を把握しておきましょう。  
...
## 第3章 サブスクリプションの編集作業を練習する

サブスクリプションの編集は頻繁に行う作業ではありません。トラブルを防ぐため、事前にApp Store Connectでの操作に慣れておくと安心です。  
そこで、ダミーのアプリを作成してApp Store Connectの操作を練習します。他のアプリとは独立しているため、操作をミスしても影響が及ぶことはありません。  
あくまでApp Store Connectでの操作を練習するのが目的なので、Xcodeで新規プロジェクトを作成する、といった作業は不要です。気軽に試せるので、一度練習することをお勧めします。  

### 2.1. 練習で設定するプランの概要

今回の練習で設定するプランは次の4種類とします。  
...
あくまで操作練習が目的ですので、プランの名前も金額も適当です。必ずしも上記と全く同じように設定する必要はありません。  

### 2.2. 練習用のアプリを作成する

1. [Apple DeveloperのIdentifiersページ](https://developer.apple.com/account/resources/identifiers/list)を開きます。
2. App IDを新規作成します。
  - Bundle IDは適当で構いません。`com.yourname.MyPaidApp`など。
  - Capabilitiesの中にあるIn-App Purchaseにチェックをつけてください。
3. [App Store ConnectのAppsページ](https://appstoreconnect.apple.com/apps)から新規アプリを追加します。
  - 新規アプリ追加時は2.で作成したBundle IDを選んでください。
  - アプリの名前は適当で構いません。「サブスク操作練習」など。
...
2. Subscription Groupsという見出しの近くにあるCreateボタンを押します。
3. Reference Nameを設定します。今回は練習ですので適当な名前で構いません。`MyPaidAppSubscription`など。
4. Createボタンを押します。

以上でSubscription Groupが新規作成されました。

#### 2.3.1 Subscription GroupのReference Nameについて

### 2.4. Subscription Groupのローカライズを行う

1. Localizationという見出しの近くにあるCreateボタンを押します。
2. Localizationボタンを押してJapaneseを選択します。
3. Display Nameを設定します。
  - Display Nameはユーザーに公開される名前です。
  - ここでは適当な名前を設定して構いません。有料プランなど。
4. App Name Display Optionsを選択します。
  - 選択肢はUse App NameとUse Custom Nameの2種類あります。
  - これは課金画面でアプリの名前をどのように表示するか制御するものです。お好みで、どちらを選んでも構いません。
5. Createボタンを押します。

以上でSubscription Groupのローカライズは完了です。なお、ローカライズしたDisplay Nameは後で変更可能です。  

### 2.5. サブスクリプションのプランを追加する

1. Subscriptionsという見出しの近くにあるCreateボタンを押します。
