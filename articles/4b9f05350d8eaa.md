...
通信に参加する者をピアとよび、サーバーを介さずピア同士で通信する方式をP2P（peer-to-peer）とよびます。WebRTC は可能な限りピア同士で通信することを前提にしたプロトコルです  
...
冒頭の`v=0`はバージョン番号を示し、`v=0`で固定です。このように、キーと値のペアが列挙される形式でSDPは記述されます。
なお、キーの種類と値について覚える必要はありません。ここではSDPの解説のために示しただけであり、通常は生のSDPを読み書きすることはしません。  
...
gitとnodeについてはバージョン依存の機能を利用しないため、多少古いバージョンでも動作するはずです。ただし、予期しないトラブルの発生を避けるため、新しいバージョンの利用がおすすめです。  
