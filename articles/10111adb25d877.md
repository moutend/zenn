...
悲しいことに、個人が開発したアプリで、いきなり莫大な収益を得られる可能性はほぼゼロです。雑所得の範囲を大きく上回り、収益が安定して継続できると見込まれる状況になるまで、開業は先送りすればよいかと思います。  

### どのような課金方式を選ぶべき？

課金の分類は2種類、消耗型と非消耗型があります。  

- 消耗型: ゲームアプリにおける体力回復アイテムのような、一度利用すると効力を失うものです。
- 非消耗型: 加入していると効力が維持されるものです。サブスクリプションは非消耗型に分類されます。  

個人でアプリを開発する場合、サブスクリプションの実装を強くお勧めします。主な理由は以下3点です。  

1. サブスクリプションの加入状態だけに気を配れば良いので実装が単純です。また、Basic / Pro / Ultraのようなグレードを用意して、排他的に選択させることがStoreKit標準機能だけで実現できます。
...
3. 消耗型の課金でよくあるトラブルを回避できます。
  - 例えばゲームアプリで体力回復アイテムを購入したのに、効果がなかったと苦情が届いたとします。考えられる原因は以下2つです。
    ...
    - ロジックの不具合: 例えば戦闘システムの実装に欠陥があり、本当に体力が回復していなかった。
...
具体的な実装の話題へ進む前に、App Store Connectにおけるサブスクリプション関連用語を把握しておきましょう。  
...
ユーザーが自らキャンセルするまで自動的に更新され続ける課金方式です。期間」として選べるのは、次の6種類です。  
...
同じSubscription Group内の相対的なプラン階層を示す整数値です。レベルは1以上の整数値で表します。  
...
基本的な機能を提供するBasicプランと、全機能が利用可能なPremiumプランの2種類があり、それぞれに月払いと年払いのプランが用意されている、という想定です。年払いは月払いの2ヶ月分を割引することにしました。  
この場合、Levelの値は以下のように設定します。  
...
契約中プランから別のプランへ移行する場合の挙動は次のようになります。  

例:  

- Basic月払いからBasic年払いに変更: 同じレベル間の変更です。日割りなし、次回の更新タイミング（翌月）にプラン変更が適用されます。
- Basic年払いからBasic月払いに変更: 同じレベル間の変更です。日割りなし、次回の更新タイミング（翌年）にプラン変更が適用されます。
...
- 下位レベルから上位レベルへのプラン変更は即座に反映されます。
-同じレベル同士のプラン変更は次回の更新タイミングで反映されます。
- 上位レベルから下位レベルへのプラン変更は次回の更新タイミングで反映されます。反映されるまでの残り期間は現在のプラン（上位レベル）が継続します。
- 月額と年額の違いはApple側でレベルではなく期間（duration）として別に管理されます。レベルで区別する必要はありません。

## サブスクリプションの編集作業を練習する

サブスクリプションの編集はそう頻繁に行う作業ではありません。トラブルを防ぐため、事前にApp Store Connectでの操作を練習しておくと安心です。  

### 練習で設定するプランの概要

今回の練習で設定するプランは次の4種類です。  
...
プランはBasicとPremiumの2種類、それぞれ月額と年額プランを用意します。年額プランは2ヶ月分安くしてみました。  

### Subscription Groupの作成

1. MonetizationメニューのSubscriptionsをクリックします。
2. Subscription Groupsという見出しの近くにあるCreateボタンを押します。
3. Reference Nameに適当な値を設定します。
