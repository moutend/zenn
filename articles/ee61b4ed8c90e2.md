...
事前に緯度と経度を求めた地点を基準点とします。ここに上記3つのiPhoneを設置して、基準点からの誤差を測定します。
...
現在地の位置情報が更新されるとデリゲート（CLLocationManagerDelegate）が呼び出されます。

注意点として、デリゲートが呼び出される間隔は一定ではありません。短い場合は1秒ほど、長い場合は20秒ほど間隔が開きます。。なるべく多くのデータを収集するため、また長時間測定を続けた場合の傾向を知るために、今回は測定時間を30分としました。
...
従って、技術進歩で測定結果の精度が5年後には1メートル単位、10年後には10センチメートル単位、そして最終的には誤差がゼロに近づく、なんて予想は的外れです。どれだけ技術が進歩しても、電波が途切れやすい場所あるいは全く電波をつかめない場所では位置情報が不正確になります。
...
絶対的な誤差が減少したのではなく、少ない誤差で測定できる確率が高まったと解釈するべきです。記事冒頭の結果について、新しい世代のiPhoneであれば古い機種と比べて正確な位置情報が得られる傾向にある、と歯切れの悪い説明をしたのはこれが理由です。
