...
- 消耗型: ゲームアプリにおける体力回復アイテムのような、一度利用すると効力を失うもの。
- 非消耗型: 加入していると効力が維持されるもの。サブスクリプションは非消耗型に分類されます。  

個人でアプリを開発する場合、サブスクリプションを強くお勧めします。主な理由は以下3点です。  
...
同じSubscription Group内の相対的なプラン階層を示す整数値です。レベルは1から999までの整数値で表します。  
レベルの値が小さいほど下位プラン、大きいほど上位プランと判断されます。Appleはこの情報を使って、ユーザーがプランを変更する際の移行ルールを自動的に制御します。  
サブスクリプションを契約中のユーザーが存在していても、任意のタイミングでレベルを変更できます。将来のブラン変更に備えて最も下位プランの初期値を1ではなく100に設定する、といった考慮は不要です。  
...
なお、月額と年額の違いはレベルではなく期間（duration）として別に管理されます。レベルで区別する必要はありません。  
...
3. Reference Nameを設定します。ここでは`MyPaidApp`と入力します。
4. Createボタンを押します。

以上でSubscription Groupが作成されました。Reference Nameの決め方については4章で補足します。  
...
作業中に気がつかれたかもしれませんが、Billing GraceやShareの設定は飛ばしました。これらの設定は行わなくてもApp Storeの審査に提出できるため、意図的に飛ばしました。詳細については5章で補足します。  
なお、本番のプラン設定では購入画面おスクリーンショットの添付とレビュー担当者へのコメントの記入が必要になります。また、プランを識別するためのアイコン画像の設定も行うとユーザーに親切です。  
