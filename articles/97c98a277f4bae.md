...
デプロイしたコードはどこのサーバーで実行されるのでしょうか？日本でしょうか、米国でしょうか？答えは世界中です。

それでは世界中のサーバーにデプロイされることを確認しましょう。リクエストのcfプロパティにはリクエストが到着したデータセンターを識別する`colo`プロパティが用意されています。
...
それでは以下のコードを`npx wrangler publish`でデプロイしてください。
...
参考までに、AWS EC2からCloudflare Workersに向けてリクエストを送信したときの結果を示します。検証を行ったリージョンは米国のバージニア北部（us-east-1a）と日本の東京（ap-northeast-1a）です。
...
| docomo回線でテザリング | `JP` | `NRT` | `9605` | `NTT Docomo` |

ご覧のとおり、リクエストの送信元に最も近いデータセンターにコードがデプロイされていることを確認できました。

さらに追加で確認を行ってみましょう。

以下にレスポンスが返ってくるまでの時間を測定した結果を示します。

もしデプロイ先が米国のみであれば日本からのリクエストは太平洋を横断することになるため、時間は伸びるはずです。
しかし、リクエストがどこから送信されたかに関わらずレスポンスタイムは均一です。
この結果から、リクエストの送信元に最も近いデータセンターが選択されており、それらのデータセンタでは同じコードが動作していることが確認できました。
...
まず、Cloudflare WorkersのJavaScriptランタイムにはV8が使われています。V8はnode.jsやChromiumで採用されているJavaScriptランタイムです。別の言い方をすれば、CloudflareのサーバーでホストされているV8をCloudflare Workersと呼んでいるのです。

V8ランタイムはfetch関数を実行するためのサンドボックスを提供します。サンドボックスはリクエストごとに作成され、メモリは共有されません。

Cloudflare WorkersはVMやコンテナの代わりにV8を採用することで軽量な仮想化を実現しています。

ブラウザのタブを開いた状況をイメージしてください。例えば、github.comを開いたタブとzenn.devを開いたタブがあるとします。それぞれのタブはお互いの通信内容をを覗き見たり、勝手にスクリプトを操作したりできません。V8は仮想化ソフトウェアではありませんが、実質的に仮想化と同等の機能を実現していると言えます。

従って、Cloudflare Workersでは共有メモリを用いたプロセス間の通信などは行えません。例えば、以下のコードをみてください。

```ts
```

（説明を簡単にするためcount変数の排他制御は考慮していません）

countはグローバル変数です。もしランタイムで共有されているならリクエストを受け取る旅にインクリメントされるはずです。確認してみましょう。
...
何度実行しても常に同じ結果が得られます。なぜならリクエストごとにサンドボックスが作成されるからです。
