...
Service bindingsを設定して、WorkerからWorkerを呼び出すように設定すると、CloudflareのダッシュボードにはConnected Worker: 1のように表示されます。

ただし、記事の執筆時点では呼び出す側のWorkerにしか接続数が表示されていません。呼び出される側のWorkerについては、、どのWorkerが自分自身を呼び出しているのか表示されていません。Pages FunctionsにService bindingsを設定した場合についても同様です。

個人的には呼び出し側と呼び出される側の両方に接続数が表示されていると嬉しいのですが、仕様ということで仕方ないのかもしれません。皆さんはConnected Worker: 0の表示を見ても焦らないよう気をつけてください。

### ローカル開発環境でのエミュレーションに未対応

`wrangler pages dev`コマンドを実行するとローカル開発環境でWorkerを起動することができます。ただし、記事の執筆時点ではService bindingsをローカル環境でエミュレートする機能が提供されていません。R2やD1などはローカル開発環境でのエミュレーションに対応しているのに、残念です。
